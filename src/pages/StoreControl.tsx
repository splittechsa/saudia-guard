...
import { 
  Sliders, Store, Brain, Clock, Bell, Cpu, 
  Plus, Trash2, GripVertical, ShieldCheck, 
  MessageSquare, Save, Zap, Info 
} from "lucide-react";
...
// ... (نفس الـ Interfaces والـ Helpers السابقة)

export default function StoreControl() {
  // ... (نفس الـ States والـ LoadData السابقة)

  return (
    <DashboardLayout>
      <div className="max-w-5xl mx-auto space-y-6 py-4" dir="rtl">
        
        {/* Header - الهوية الاحترافية */}
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground font-arabic mr-10">{store.name} — تخصيص معايير الرقابة والتدقيق</p>
          </div>
        ...
        <AnimatePresence mode="wait">
                ...
                <div className="glass-strong rounded-3xl p-8 border border-border relative overflow-hidden">
                  ...
                  <div className="space-y-3 mb-8">
                    {questions.map((q, idx) => (
                      <motion.div 
                        key={idx} 
                        layout
                        className="group rounded-2xl border border-border bg-secondary/30 hover:border-primary/30 transition-all overflow-hidden"
                      >
                  ...
                  <div className="flex gap-2">
                    <Input 
                      ...
                      placeholder="مثال: هل يرتدي الموظفون الزي الرسمي؟"
                      className="h-12 rounded-xl bg-secondary border-border font-arabic"
                    />
                ...
                <div className="glass-strong rounded-3xl p-6 border border-border">
                  ...
                  <ul className="space-y-4 text-xs text-muted-foreground font-arabic leading-relaxed">
                    <li className="flex gap-2">
                       ...
                       <span>اكتب أسئلة واضحة يمكن الإجابة عليها من خلال الرؤية (نعم/لا أو وصف قصير).</span>
                    </li>
                    <li className="flex gap-2">
                       <span className="text-primary font-bold">2.</span>
                       <span>سيقوم محرك سبلت تيك بتحليل آخر 10 دقائق من البث للإجابة بدقة.</span>
                    </li>
                    <li className="flex gap-2">
                       <span className="text-primary font-bold">3.</span>
                       <span>النتائج ستظهر فوراً في لوحة البيانات وتُرسل في التقرير اليومي.</span>
                    </li>
                  </ul>
                </div>
          ...
          {/* ... (باقي التبويبات بأسلوب الـ glass-strong والـ rounded-3xl) */}
        </AnimatePresence>

...
export default function PaymentPage() {
    ...
    window.goSell.config({
      ...
      gateway: {
        ...
        callback: async (response: any) => {
          ...
          if (
            response.callback?.status === "CAPTURED" ||
            response.callback?.id
          ) {
            await processPayment(response.callback.id);
          } else {
            setLoading(false);
  ...
  const processPayment = async (chargeId: string) => {
    ...
    try {
      ...
      if (data?.status === "CAPTURED" || data?.redirect_url) {
        ...
        toast.error("حالة الدفع غير مؤكدة، سيتم التحقق تلقائياً");
        navigate("/dashboard/payment-success");
      }
    } catch (err: any) {
  ...
  return (
    <div
      className="min-h-screen bg-background carbon-grid relative overflow-hidden"
      dir="rtl"
    >
      ...
      <div className="max-w-lg mx-auto px-4 py-12">
        ...
        >
          <h3 className="text-sm font-semibold text-foreground mb-4 font-arabic">
            ملخص الطلب
          </h3>
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-sm font-bold text-foreground">
              {Math.round(price * 0.15)} ر.س
            </span>
          </div>
          <div className="border-t border-border my-3" />
          <div className="flex justify-between items-center">
            <span className="text-base font-bold text-foreground font-arabic">
              الإجمالي
            </span>
            <span className="text-lg font-bold text-primary">
              {Math.round(price * 1.15)} ر.س
            </span>
          </div>
        </motion.div>
        ...
        >
          ...
          >
            {verifying ? (
              ...
              <>
                <Lock className="w-4 h-4 me-2" /> ادفع{" "}
                {Math.round(price * 1.15)} ر.س
              </>
            )}
          </Button>
